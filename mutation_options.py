...
class Instructions:
    ...
    # TODO this requires some debugging to work with the current state!
    def prune(self):
        '''
...
def perform_folding(hamiltonian, circuit):
    ...
    for gate in gates:
        # print("\t folding", gate)
        param_hamiltonian = fold_unitary_into_hamiltonian(H=param_hamiltonian, gate=gate)
...
def evaluate_fitness(instructions, hamiltonian: tq.QubitHamiltonian, type_energy_eval: str, cluster_circuit: tq.QCircuit=None) -> tuple:
    ...
    print('vars extracted', clifford_circuit.extract_variables())
    raise Exception("here, we need to make sure that things still work with wfn opti and that static parameters are not accounted for!.", flush=True)
    initial_guess = None

    if not parametrized:
        # folded_hamiltonian = (convert_PQH_to_tq_QH(folded_hamiltonian))()
        print('not sure yet what it means that not parametrized')
